...
if(isset($_POST["submit"])){
    ...
    if(empty($username) || empty($email) || empty($betreff) || empty($nachricht) ){
        header("Location: ../index.php?error=emptyfields&uid=".$username."&mail=".$email);
        exit();
    }
    else if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/",$username)){
        header("Location: ../index.php?error=invaliduid&mail=");
        exit();
    }
    else{
        ...
        $email_from = "absender@domain.ch";   //Absender falls keiner angegeben wurde
        $sendermail_antwort = true;      //E-Mail Adresse des Besuchers als Absender. false= Nein ; true = Ja
        $name_von_emailfeld = "Email";   //Feld in der die Absenderadresse steht

        $empfaenger = "rhottana.truy@gmail.com"; //Empfänger-Adresse
        $mail_cc = ""; //CC-Adresse, diese E-Mail-Adresse bekommt einer weitere Kopie
        $betreff = "Neue Kontaktanfrage"; //Betreff der Email

        $url_ok = "../index.php?error=success"; //Zielseite, wenn E-Mail erfolgreich versendet wurde
        $url_fehler = "kontaktformular_fehler.inc.php"; //Zielseite, wenn E-Mail nicht gesendet werden konnte
